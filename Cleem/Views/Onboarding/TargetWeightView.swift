...
struct TargetWeightView: View {
    ...
    private var weightChangeText: String {
        ...
        let absChange = abs(change).rounded(to: 1)
        if change > 0 {
    ...
    private var weightChangePercentage: String {
        ...
        let change = displayWeight - currentWeight
        let percentage = (change / currentWeight * 100).rounded(to: 1)
        ...
        ZStack {
            ...
            VStack(spacing: 0) {
                ...
                VStack(spacing: 30) {
                    ...
                    VStack(spacing: 20) {
                        // Quick adjust segments - final approach
                        ZStack {
                            ...
                            HStack(spacing: 0) {
                                ...
                                Button(action: {
                                    ...
                                    Text("Lose weight")
                                        .font(.system(size: 15, weight: selectedSegment == 0 ? .bold : .medium))
                                        .foregroundColor(selectedSegment == 0 ? Color.red.opacity(1.0) : .black.opacity(0.7))
                                        .frame(maxWidth: .infinity)
                                        .frame(height: 44)
                                        .background(selectedSegment == 0 ? Color.red.opacity(0.25) : Color.clear)
                                }
                                .contentShape(Rectangle())
                                ...
                                Button(action: {
                                    ...
                                    Text("Maintain")
                                        .font(.system(size: 15, weight: selectedSegment == 1 ? .bold : .medium))
                                        .foregroundColor(selectedSegment == 1 ? Color(red: 0.0, green: 0.3, blue: 0.7) : .black.opacity(0.7))
                                        .frame(maxWidth: .infinity)
                                        .frame(height: 44)
                                        .background(selectedSegment == 1 ? Color(red: 0.75, green: 0.85, blue: 1.0) : Color.clear)
                                }
                                .contentShape(Rectangle())
                                ...
                                Button(action: {
                                    ...
                                    Text("Gain weight")
                                        .font(.system(size: 15, weight: selectedSegment == 2 ? .bold : .medium))
                                        .foregroundColor(selectedSegment == 2 ? Color.green.opacity(1.0) : .black.opacity(0.7))
                                        .frame(maxWidth: .infinity)
                                        .frame(height: 44)
                                        .background(selectedSegment == 2 ? Color.green.opacity(0.25) : Color.clear)
                                }
                                .contentShape(Rectangle())
                            }
                        }
    ...
    private func quickAdjustButton(title: String, isSelected: Bool, color: Color, textColor: Color, backgroundColor: Color, action: @escaping () -> Void) -> some View {
        Button(action: action) {
            Text(title)
                ...
                .background(
                    Rectangle()
                        .fill(isSelected ? backgroundColor : Color.clear)
                )
                .foregroundColor(isSelected ? textColor : .black.opacity(0.7))
    ...
    private func syncGoalProperties() {
        ...
        case .gainMuscle:
            ...
            if targetWeight <= currentWeight {
                ...
                targetWeight = (currentWeight * 1.05).rounded(to: hapticNotch)
            }
        }
    }
    ...
    private func setSegmentBasedOnGoal() {
        ...
        case .loseWeight:
            ...
            let currentWeight = navigationCoordinator.userProfile.weightInKg
            if targetWeight >= currentWeight {
                targetWeight = (currentWeight * 0.95).rounded(to: hapticNotch)
            }
            
        case .maintainWeight:
            selectedSegment = 1 // Maintain
            targetWeight = navigationCoordinator.userProfile.weightInKg.rounded(to: hapticNotch)
    ...
    private func adjustWeight(_ percentage: Double) {
        ...
        targetWeight = (currentWeight + change).rounded(to: hapticNotch)
